...
const normalizeStartup = (raw: Partial<Startup>): Startup => {
  ...
  return {
    ...
    businessModel: typeof raw.businessModel === 'string' ? raw.businessModel : undefined,
    tractionDetails: typeof raw.tractionDetails === 'string' ? raw.tractionDetails : undefined,
    competitiveAdvantage: typeof raw.competitiveAdvantage === 'string' ? raw.competitiveAdvantage : undefined,
    foundingStory: typeof raw.foundingStory === 'string' ? raw.foundingStory : undefined,
    vision: typeof raw.vision === 'string' ? raw.vision : undefined,
    currentChallenges: typeof raw.currentChallenges === 'string' ? raw.currentChallenges : undefined,
    ...
    valuationUsd: typeof raw.valuationUsd === 'number' ? raw.valuationUsd : undefined,
...
const buildStartupListing = (startup: Startup) => ({
  ...
  targetCustomer: startup.targetCustomer || null,
  businessModel: startup.businessModel || null,
  tractionDetails: startup.tractionDetails || null,
  competitiveAdvantage: startup.competitiveAdvantage || null,
  foundingStory: startup.foundingStory || null,
  vision: startup.vision || null,
  currentChallenges: startup.currentChallenges || null,
  ...
  valuationUsd: startup.valuationUsd || null,
  currencyCode: startup.currencyCode || null,
...
export default function App() {
  ...
  // Network connection requests are stored with both user IDs in
  // participants. Query that same field so Firestore can prove the rule
  // condition before returning any document.
  useEffect(() => {
  ...
  // Appointments also persist both participants on every record. Use the
  // participant query so the security rule and query constraint are identical.
  useEffect(() => {
    const userId = typeof currentUser.id === 'string' ? currentUser.id.trim() : '';
