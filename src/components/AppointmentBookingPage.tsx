            ...
            <div>
              ...
              <div className="mornai-booking-summary mt-5 rounded-2xl p-4">
                <div className="flex items-center gap-2 text-xs font-bold text-violet-700">
                  <Sparkles className="h-4 w-4" /> AI fit brief
                </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[.15em] text-violet-200">Founder sync</span>
                  <span className="flex items-center gap-1 text-[11px] font-semibold text-slate-300"><MapPin className="h-3 w-3" /> {startup.location}</span>
                </div>
          ...
          <form onSubmit={submit} className="space-y-5">
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-xl font-extrabold text-slate-950">Choose how you want to start the conversation.</h2>
              </div>
            ...
            <div>
              ...
              <select value={roleTitle} onChange={(e) => setRoleTitle(e.target.value)} className="mornai-booking-field w-full rounded-2xl px-4 py-3.5 text-sm font-semibold text-slate-800 outline-none">
                {openRoles.map((role) => <option key={role.id} value={role.title}>{role.title} • {role.type}</option>)}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <div className="relative">
                  <Calendar className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-violet-500" />
                  <input type="date" required min={getToday()} value={date} onChange={(e) => setDate(e.target.value)} className="mornai-booking-field w-full rounded-2xl py-3.5 pl-11 pr-4 text-sm font-semibold text-slate-800 outline-none" />
                </div>
              </div>
              <div>
                <label className="mb-2 block text-xs font-extrabold text-slate-600">Time slot</label>
                <div className="relative">
                  <Clock3 className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-violet-500" />
                  <select value={time} onChange={(e) => setTime(e.target.value)} className="mornai-booking-field w-full rounded-2xl py-3.5 pl-11 pr-4 text-sm font-semibold text-slate-800 outline-none">
                    {['10:00','11:30','14:00','16:30'].map((slot) => <option key={slot}>{slot}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea rows={6} required value={pitchMessage} onChange={(e) => setPitchMessage(e.target.value)} placeholder="Explain what you can contribute and what you want to help build." className="mornai-booking-field w-full rounded-2xl px-4 py-3.5 text-sm leading-6 text-slate-800 outline-none" />
              <p className="mt-2 text-[11px] text-slate-400">MornAI attaches this to the founder's preparation brief.</p>
            </div>

            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
              <div className="flex items-center gap-2 text-xs text-slate-500"><Video className="h-4 w-4 text-violet-500" /> Integrated sync details are created with the request.</div>
              <button type="submit" className="mornai-primary-action inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-extrabold text-white"><Send className="h-4 w-4" /> Send sync request</button>
            </div>
          </form>

          <aside className="space-y-4">
            <section className="mornai-booking-side rounded-[24px] p-5">
              <div className="flex items-center gap-2"><BriefcaseBusiness className="h-4 w-4 text-violet-600" /><h3 className="text-sm font-extrabold text-slate-950">Open opportunities</h3></div>
              <div className="mt-4 space-y-3">
                {openRoles.slice(0, 4).map((role) => (
                  <button key={role.id} type="button" onClick={() => setRoleTitle(role.title)} className={`w-full rounded-2xl border p-4 text-left transition-all ${roleTitle === role.title ? 'border-violet-300 bg-violet-50/80 shadow-[0_14px_34px_rgba(124,58,237,.10)]' : 'border-slate-200 bg-white/75 hover:border-violet-200 hover:bg-white'}`}>
                    <div className="flex items-start justify-between gap-3"><b className="text-sm text-slate-900">{role.title}</b><span className="rounded-full bg-violet-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[.12em] text-violet-700">{role.type}</span></div>
                    <p className="mt-2 text-[11px] leading-5 text-slate-500 line-clamp-3">{role.description}</p>
                    <div className="mt-3 flex flex-wrap gap-1.5">{(role.skills || []).slice(0, 4).map((skill) => <span key={skill} className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-600">{skill}</span>)}</div>
                  </button>
                ))}
              </div>
            </section>
            <section className="rounded-[24px] border border-emerald-100 bg-emerald-50/75 p-5">
              <div className="flex items-center gap-2 text-xs font-extrabold text-emerald-800"><CheckCircle2 className="h-4 w-4" /> Founder context included</div>
              <p className="mt-2 text-xs leading-6 text-emerald-700">Your profile, selected role and pitch travel together so the founder gets context instead of another mystery calendar invite.</p>
            </section>
          </aside>
        </div>
      </div>
